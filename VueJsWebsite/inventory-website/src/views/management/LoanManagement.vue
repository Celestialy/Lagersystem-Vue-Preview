<template>
  <b-container>
          ...
          <div class="tablecontainer" style="text-align:center">
            ...
            >
              ...
              <template v-slot:cell(amountLeft)="data">
                <p
                  v-if="(data.item.amountLeft/data.item.barcodes.length) * 100 > 25"
                >{{ data.item.amountLeft }}/{{ data.item.barcodes.length }}</p>
                <p
                  v-if="(data.item.amountLeft/data.item.barcodes.length) * 100 <= 25 && (data.item.amountLeft/data.item.barcodes.length) * 100 > 10"
                  style="darkOrange"
                >{{ data.item.amountLeft }}/{{ data.item.barcodes.length }}</p>
                <p
                  v-if="(data.item.amountLeft/data.item.barcodes.length) * 100 <= 10"
                  style="color:red"
                >{{ data.item.amountLeft }}/{{ data.item.barcodes.length }}</p>
              </template>
                  ...
                  <b-row>
                    ...
                    <b-col lg="3">
                      <b-button
                        ...
                        :disabled="data.item.barcodes.length > 0" 
                        :title="data.item.barcodes.length > 0? 'Der er stadig stejkoder' : ''"
                      >Slet</b-button>
                    </b-col>
                  </b-row>
                </div>
    ...
    <AddItem :SelectedImage="SelectedImage" :Categories="Categories" @GetItems="getItems()" :isLoan="true" @show="SelectedImage = null"></AddItem>
...
export default {
      ...
      get() {
        if (this.selectedImage != null) {
          ...
          return {
            ...
            imageUri:
              process.env.VUE_APP_IMAGEPLACEHOLDER
          }
        }
      },
      set(value) {
  ...
  methods: {
    ...
    getItems() {
      ...
      this.$store
        .dispatch('inventoryManagement/getLoanItems', 1)
        .then(() => {
          // eslint-disable-next-line no-console
          console.log(
            'Got some data, now lets show something in this component'
          )
          this.totalRows = this.LoanItemAmount
          this.Items = this.LoanItems
